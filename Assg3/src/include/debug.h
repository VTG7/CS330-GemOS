...
// !my modification. To maintain the list of function present on the stack
struct fun_on_stack {
	u64 addr;	// address of the current function
	struct fun_on_stack * next; // pointer to the next node
	u64 original_return_addr; // the original return address to return to
};
/*
...
struct breakpoint_info{
	...
	int is_on_stack; // ! my modification. Will notify if the function is on the stack(only for end_breakpoint_enabled functions)
};
...
struct debug_info{
	...
	u32 cpid; 	//! my addition. Stores the pid of the child(debugee)
	u32 breakpoint_count;		// number of breakpoints present
	u32 breakpoint_mastercount; // ! my modification. Will include the total breakpoints ever created
	struct breakpoint_info *head;   // head of breakpoint list
	void *end_handler;
	struct fun_on_stack * next_fun; // !my modification. To have the list of the functions in the stack
};
